...
\end{tikzpicture}} of $K$ can be smoothed in two different ways. The first way is to smooth it horizontally \raisebox{-.07cm}{\begin{tikzpicture}[scale=.2]
\draw [line width=0.35mm] plot [smooth, tension=2] coordinates { (-1,.8) (0, 0.5) (1,.8)};
\draw [ line width=0.35mm] plot [smooth, tension=2] coordinates { (-1,-.8) (0, -0.5) (1,-.8)};
\end{tikzpicture}} and the second is to smooth it vertically  \raisebox{-.1cm}{\begin{tikzpicture}[scale=.2]
...
\subsubsection{\label{sec:theory:jones:dbc}Double branched cover invariant}
This invariant is defined for planar knotoids. To a planar knotoid $k$ one associates its double branched cover, which is a knot living in a solid torus. The double branched cover invariant of $k$ is then the Jones polynomial of this knot in the solid torus.

Because the ambient space is a solid torus rather than the plane or the sphere, the resulting polynomial carries an extra variable. In addition to the Kauffman bracket variable $A$, and much like the Turaev loop bracket, a second variable $v$ records how the components of a smoothing wrap around the core of the solid torus. The invariant is therefore a Laurent polynomial in the two variables $A$ and $v$. As for the other bracket polynomials, it is evaluated as a state sum over the $2^n$ smoothings of a diagram with $n$ crossings and is thus exponential in the number of crossings.

The double branched cover invariant is computed by \lstinline{polynomial_invariant} with the option \lstinline{--double-branched-cover} (which requires \lstinline{--planar} and \lstinline{--closure-method=open}), and from the Python module through the function \lstinline{double_branched_cover_polynomial} (see section ``\ref{sec:python} \nameref{sec:python}'').
