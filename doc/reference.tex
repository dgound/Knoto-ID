...
\item[\lstinline{--double-branched-cover}]\hfill\\
  Evaluate the double branched cover invariant instead of the Turaev loop bracket. This invariant is the Jones polynomial (in variables $A$ and $v$) of the double branched cover of the planar knotoid, which is a knot in a solid torus\footnote{see section ``\ref{sec:theory:jones:dbc} \nameref{sec:theory:jones:dbc}'' for more details.}. It is only defined for planar knotoids, so this option requires \lstinline{--planar} and an open curve (\lstinline{--closure-method=open}). It cannot be combined with \lstinline{--arrow-polynomial}.
