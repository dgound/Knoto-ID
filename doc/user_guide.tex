...
\section*{What's new in version \cmakeversion}
\addcontentsline{toc}{section}{What's new in version \cmakeversion}

Version \cmakeversion\ of {\it Knoto-ID} introduces two main additions.

\begin{itemize}
\item {\bf Double branched cover invariant for planar knotoids.} The executable \lstinline{polynomial_invariant} can now compute the double branched cover invariant. It is obtained from the double branched cover of a planar knotoid, which is a knot in a solid torus, and is expressed as a polynomial in the variables $A$ and $v$. It is enabled with the new option \lstinline{--double-branched-cover} and is only defined for open curves projected on a plane (options \lstinline{--closure-method=open} and \lstinline{--planar}). See the option description in section ``\ref{sec:reference} \nameref{sec:reference}'' and the definition in section ``\ref{sec:theory:jones:dbc} \nameref{sec:theory:jones:dbc}''.
\item {\bf Python module.} {\it Knoto-ID} now ships with an optional Python module that exposes its core functionality to Python. It gives access to the polynomial invariants, the knotted core and all-subchains analysis, reading and writing PD codes, the double branched cover invariant, plotting of fingerprint and disk matrices, and interoperability with Knoodle. See section ``\ref{sec:python} \nameref{sec:python}''.
\end{itemize}
